  ...
  return (
    <div className="grid grid-cols-4 gap-3 p-3">
      {panelStatuses.map((panel) => (
        <Card
          key={panel.id}
          className="flex flex-col items-center justify-center p-2 text-xs hover:cursor-pointer hover:bg-green-700"
          onClick={() => {
            handleSetCurrentPanel(panel.id);
          }}
        >
          <CardDescription className="text-center">#{panel.id}</CardDescription>
